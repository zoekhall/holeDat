...
function Feed() {
  ...
  const getAllImgs = () => { // gets all images of all potholes
    axios
      .get('/api/imgs')
  ...
  const sortByNew = () => { // sorts the current filter by new
  ...
  const sortByUnique = () => {
    ...
    let resultArr = globalFeed.filter(val => {
      if (!filtArr.includes(val.potholePotholeId)) { // if the potholePotholeId has not already been seen
        filtArr.push(val.potholePotholeId)
        return true
      }
    })
    setGlobalFeed([...resultArr])
