...
const Rating = () => {
  ...
  const ratePothole = () => {
    axios.post('/api/rating/fromPh', { id, fixed, rating, user })
  ...
  const handleCone = i => {
    let x = 4
    while (x >= 0) {
      document.getElementById(`cone-num-${x}`)?.classList.remove('clickCone')
      x--
    }
    setRating(i + 1)
    while (i >= 0) {
      document.getElementById(`cone-num-${i}`)?.classList.add('clickCone')
      i--
    }
